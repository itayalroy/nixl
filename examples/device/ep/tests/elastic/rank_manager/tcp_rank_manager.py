...
class RankManager(RankManagerBase):
    ...
    def get_rank(self) -> Tuple[int, int, Optional[str]]:
        ...
        local_rank = int(server_response[0])
        global_rank = int(server_response[1])
    ...
    def release_rank(self, user_context: Optional[str] = None) -> bool:
        s = socket.create_connection((self._server, self._port))
        s.sendall(
            f"RELEASE_RANK {self._global_rank if self._global_rank is not None else -1} {user_context}\n".encode()
        )
