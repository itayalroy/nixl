...
void Buffer::connect_ranks(const std::vector<int>& remote_ranks_list, const std::optional<std::vector<nixl_blob_t>>& remote_mds) {
    ...
    _nixl_ep_connect_ranks(new_ranks);
...
void Buffer::disconnect_ranks(const std::vector<int>& remote_ranks_list) {
    ...
    _nixl_ep_disconnect_ranks(remote_ranks_list);
...
void Buffer::_nixl_ep_connect_ranks(const std::vector<int>& ranks) {
    /* Elasticity is not supported until NIXL supports updateMemoryView API */
    EP_HOST_ASSERT(gpu_ctx.local_mvh == nullptr && gpu_ctx.remote_mvh == nullptr && gpu_ctx.barrier_mvh == nullptr);
    ...
    local_descs.addDesc(nixlBlobDesc((uintptr_t)(local_barrier_cnt_ptr), max_num_ranks * sizeof(int), get_local_device_id(), ""));
    ...
    for (int r = 0; r < max_num_ranks; r++) {
        ...
        int idx = (r == rank) ? (r + 1) % max_num_ranks : r;
        remote_descs.addDesc(nixlRemoteDesc((uintptr_t)nixl_peer_info[idx].rdma_buffer_ptr, num_rdma_bytes, nixl_peer_info[idx].device_id, nixl_agent_info->remote_agent_names[idx]));
        barrier_descs.addDesc(nixlRemoteDesc((uintptr_t)nixl_peer_info[idx].sync_buffer_ptr, max_num_ranks * sizeof(int), nixl_peer_info[idx].device_id, nixl_agent_info->remote_agent_names[idx]));
    }

    nixl_opt_args_t opt_args;
    opt_args.backends.push_back(nixl_agent_info->backend);
    EP_HOST_ASSERT(nixl_agent_info->agent->prepMemoryView(local_descs, gpu_ctx.local_mvh, &opt_args) == NIXL_SUCCESS);
    EP_HOST_ASSERT(nixl_agent_info->agent->prepMemoryView(remote_descs, gpu_ctx.remote_mvh, &opt_args) == NIXL_SUCCESS);
    EP_HOST_ASSERT(nixl_agent_info->agent->prepMemoryView(barrier_descs, gpu_ctx.barrier_mvh, &opt_args) == NIXL_SUCCESS);
}

void Buffer::_nixl_ep_disconnect_ranks(const std::vector<int>& ranks) {
    /* Once updateMemoryView API is supported, for rank in ranks, remove rank's buffers from the memory views */
    EP_HOST_ASSERT(0);
}
...
void Buffer::_nixl_ep_init(void) {
    ...
    gpu_ctx.local_barrier_buffer = sync_buffer_ptr;
    gpu_ctx.local_barrier_cnt = local_barrier_cnt_ptr;
    gpu_ctx.num_channels = num_ucx_channels;
    gpu_ctx.num_ranks = num_ranks;
    gpu_ctx.rank = rank;

    /* Memory views should be initialized here once NIXL supports updateMemoryView API */
...
void Buffer::_nixl_ep_destroy(void) {
    nixl_agent_info->agent->releaseMemoryView(gpu_ctx.local_mvh);
    nixl_agent_info->agent->releaseMemoryView(gpu_ctx.remote_mvh);
    nixl_agent_info->agent->releaseMemoryView(gpu_ctx.barrier_mvh);
    gpu_ctx.local_mvh = nullptr;
    gpu_ctx.remote_mvh = nullptr;
    gpu_ctx.barrier_mvh = nullptr;
}
